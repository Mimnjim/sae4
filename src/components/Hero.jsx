...
// //         // Gestion du chargement : on attend que les 2 scènes 3D soient prêtes + les 2 HUDs
// //         // 1. Compteurs
// //         const [loadedCount, setLoadedCount] = useState(0); // pour 3D
// //         const [hudReady, setHudReady] = useState({ akira: false, gis: false }); // pour HUDs
...
// //         // 3. Callbacks HUD
// //         const handleAkiraHUDReady = () => setHudReady(prev => ({ ...prev, akira: true }));
// //         const handleGisHUDReady   = () => setHudReady(prev => ({ ...prev, gis: true }));

// //         // 4. Lancer le timeline quand tout est prêt
// //         useEffect(() => {
// //             if (loadedCount >= 2 && hudReady.akira && hudReady.gis) {
// //                 setTimeout(buildTimeline, 300);
// //             }
// //         }, [loadedCount, hudReady, buildTimeline]);
...
//     const built        = useRef(false);
//     const readyCount   = useRef(0); // compte les 4 ready (2 × 3D + 2 × HUD)

//     const [loadedCount, setLoadedCount] = useState(0);

//     // ── Remise à zéro scroll ───────────────────────────────────
//     useEffect(() => {
//         window.history.scrollRestoration = 'manual';
//         window.scrollTo(0, 0);
//     }, []);

//     // ── Animation flottante scroll-down ────────────────────────
//     // Lancée dans un useEffect pour être sûr que le DOM est monté,
//     // stockée dans un ref pour pouvoir la pauser au scroll.
...
//         gsap.set(
//             [sloganRef.current, buttonsRef.current, scrollDownRef.current, footerHeroRef.current],
//             { opacity: 1, y: 0, visibility: 'visible' }
//         );
//         gsap.set(akiraHeroRef.current,  { x: 0, opacity: 1, visibility: 'visible', scale: 1 });
//         gsap.set(gisHeroRef.current,    { x: 0, y: -30, opacity: 1, visibility: 'visible', scale: 1 });
...
//         }, 0);
//         tl.to(
//             [sloganRef.current, buttonsRef.current, footerHeroRef.current],
...
//         // PHASE 7 : SORTIE GIS
//         tl.to(overlayGisRef.current, { opacity: 0, pointerEvents: 'none', duration: 0.5 }, 9.0);
//         tl.to(gisHeroRef.current,    { opacity: 0, pointerEvents: 'none', duration: 0.5 }, 9.0);

//         // PHASE 8 : OVERLAY NOIR — fade simple, GPU only
//         tl.to(transitionRef.current, {
//             opacity: 1,
//             pointerEvents: 'auto',
//             duration: 2,
...
//         // Quand le Hero se dépinne → l'overlay se dissout
//         ScrollTrigger.create({
//             trigger: heroRef.current,
//             start: 'bottom bottom',
//             onEnter: () => {
...
//                     delay: 0.05,
//                     onComplete: () => {
//                         if (transitionRef.current) transitionRef.current.style.pointerEvents = 'none';
//                     }
//                 });
//             },
//             onLeaveBack: () => {
//                 gsap.set(transitionRef.current, { opacity: 1, pointerEvents: 'auto' });
//             }
//         });
...
//             }
//         });
//     }, []); // ← dépendances vides : buildTimeline ne change jamais

//     // ── Gestion du chargement ──────────────────────────────────
//     // On utilise un ref counter plutôt que du state pour éviter
//     // les re-renders multiples qui perturbent les modèles 3D.
//     const handleAkiraReady = useCallback(({ camera, initialZ, model }) => {
//         akiraCam.current  = { camera, initialZ };
//         akiraModel.current = model;
//         setLoadedCount(prev => prev + 1);
...
//     // HUD onReady : utilise un ref counter pour ne pas re-render
...
//     // Fallback : si les HUD onReady ne déclenchent pas (timing)
//     useEffect(() => {
//         if (loadedCount >= 2) {
...
//     return (
//         <>
//             {/* Overlay de transition — élément vide, GPU opacity only */}
//             <div
//                 ref={transitionRef}
//                 style={{
//                     position:      'fixed',
